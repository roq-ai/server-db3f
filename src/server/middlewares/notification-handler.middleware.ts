...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'order.create.create': {
    roles: ['admin', 'moderator', 'registered-user'],
    key: 'order_id',
    tenantPath: [],
    userPath: [],
  },
  ...
  'support.create.create': { roles: ['admin', 'moderator'], key: 'ticket_id', tenantPath: [], userPath: [] },
};

const ownerRoles: string[] = ['admin'];
const customerRoles: string[] = ['guest'];
const tenantRoles: string[] = ['admin', 'moderator', 'registered-user', 'guest-user'];
