...
import { TagListPage as ShowCaseList } from 'pages/tags';
...
function DashboardPage() {
  ...
  const tagStat = useSWR<PaginatedInterface<TagInterface>>(
    () => `/tags`,
    () => getTags({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Tags',
      result: tagStat?.data?.totalCount,
      isLoading: tagStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`tags`} passHref legacyBehavior>
                  <Button
